---
name: kingdee-design
description: 金蝶设计（Kingdee-Design）——基于 Kingdee-Design 原创框架、注入金蝶品牌色体系的 HTML 高保真原型、交互Demo、幻灯片、设计变体探索一体化设计能力。默认品牌规范：主色 #2970ff（科技蓝），渐变 135° #2970ff→#2cbdff，辅助色包含深海蓝 #0d172e、天蓝 #00a3f4、青蓝 #2cbdff、冰蓝 #eef5ff、紫罗兰 #7858f9、琥珀 #ffb200 等。HTML是工具不是媒介，根据任务 embody 对应专家（UX设计师/幻灯片设计师/原型师）。触发词：做原型、设计Demo、交互原型、HTML演示、设计变体、hi-fi设计、UI mockup、prototype、设计探索、做个HTML页面、app原型、iOS原型、移动应用mockup、设计风格、设计方向、设计哲学、配色方案、视觉风格、推荐风格、选个风格、做个好看的、评审、好不好看、review this design。**主干能力**：Junior Designer工作流（先给假设+reasoning+placeholder再迭代）、反AI slop清单、React+Babel最佳实践、Tweaks变体切换、Speaker Notes演示、Starter Components（幻灯片外壳/变体画布/设备边框）、App原型专属守则（默认从Wikimedia/Met/Unsplash取真图、每台iPhone包AppPhone状态管理器可交互、交付前跑Playwright点击测试）、Playwright验证。**需求模糊时的Fallback**：设计方向顾问模式——从5流派×20种设计哲学推荐3个差异化方向，并行生成3个视觉Demo让用户选。**交付后可选**：专家级5维度评审（哲学一致性/视觉层级/细节执行/功能性/创新性各打10分+修复清单）。
---
...
本 Skill 继承 Kingdee-Design 全部设计能力与工作流，**默认品牌规范已锁定为金蝶色板**。用户未指定其他品牌时，所有 HTML 产出的 CSS 变量直接从以下色值注入：

```css
:root {
  --brand-primary: #2970ff;
  --brand-gradient: linear-gradient(135deg, #2970ff, #2cbdff);
  --brand-deep-navy: #0d172e;
  --brand-sky: #00a3f4;
  --brand-dark: #3a3c46;
  --brand-cyan: #2cbdff;
  --brand-silver: #b3b8c5;
  --brand-turquoise: #00e5e5;
  --brand-ice: #eef5ff;
  --brand-lavender: #7858f9;
  --brand-amber: #ffb200;
  --brand-white: #FFFFFF;
}
```

**用户指定其他品牌时**：走「核心资产协议」正常采集该品牌的色值，覆盖默认值。**未指定品牌时**：禁止临场发明新颜色，以上金蝶色板是唯一色源。

**HTML是工具，但你的媒介和产出形式会变**——做幻灯片时别像网页，做App原型时别像说明书。**根据任务embody对应领域的专家**：UX设计师/幻灯片设计师/原型师。
...
**固定尺寸内容**（幻灯片）必须自己实现JS缩放，用auto-scale + letterboxing。
...
| **按输出类型查场景模板**（封面/PPT/网页） | `references/scene-templates.md` |
...
- **文件夹输出**：每个项目必须是一个独立文件夹，统一存放在 `~/Downloads/` 下
- **命名规范**：文件夹名必须为 `kingdee-design-<project-name>`，项目名称用英文或拼音，如 `kingdee-design-cangqiong-ppt`、`kingdee-design-ios-mockup`
- HTML 文件命名描述性：`Landing Page.html`、`iOS Onboarding v2.html`
- 大改版时 copy 一份旧版保留：`My Design.html` → `My Design v2.html`
- 避免 >1000 行的大文件，拆成多个 JSX 文件 import 进主文件
- 幻灯片等固定尺寸内容，**播放位置**存 localStorage——刷新不丢
- 最终产出用浏览器打开检查或用 Playwright 截图

## 核心提醒

- **事实验证先于假设**（核心原则 #0）：涉及具体产品/技术/事件（DJI Pocket 4、Gemini 3 Pro 等）必须先 `WebSearch` 验证存在性和状态，不凭训练语料断言。
- **Embody专家**：做原型时是UX设计师，做幻灯片时是幻灯片设计师。不是写Web UI。
