...
## 金蝶色系适配优先级（本 Skill 第一选择逻辑）

> **触发条件**：用户需求模糊、未指定其他品牌、走「设计方向顾问模式」推荐风格时，**第一优先级是适配金蝶品牌色系**，而非盲选通用风格。

金蝶品牌色系核心：
- 主色 `#2970ff`（科技蓝）
- 渐变 `135° #2970ff → #2cbdff`（蓝→青蓝）
- 辅助色：深海蓝 `#0d172e`、天蓝 `#00a3f4`、青蓝 `#2cbdff`、冰蓝 `#eef5ff`、紫罗兰 `#7858f9`、琥珀 `#ffb200`
...
| 风格 | 原因 | 金蝶色注入方式 |
|------|------|---------------|
| **04 Fathom** | 冷静专业色调（灰、海军蓝），天然兼容科技蓝 | 主色用 `#2970ff`，背景 `#0d172e`，辅助用冰蓝 |
| **06 Active Theory** | 霓虹渐变系，原生蓝/青/深空配色 | 直接用金蝶渐变 `#2970ff→#2cbdff`，深空底 `#0d172e` |
| **16 Territory Studio** | FUI 橙/琥珀或青系，青色版本完美适配 | 用天蓝 `#00a3f4` + 青蓝 `#2cbdff` 做 HUD accent |
| **17 Takram** | 柔和科技感，中性自然色系 | 主色 `#2970ff`，背景用暖灰/米白，科技蓝做精致 accent |

#### ★★☆ 中适配（克制单色，可用金蝶蓝做 accent）

| 风格 | 原因 | 金蝶色注入方式 |
|------|------|---------------|
| **01 Pentagram** | 克制配色（黑白+1个品牌色），accent 位置天然留给品牌主色 | 黑白基底，accent 改用 `#2970ff`，保留瑞士网格 |
| **05 Locomotive** | 深色电影感，accent 灯光可替换 | 深底 `#0d172e`，glow accent 用青蓝 `#2cbdff` |
| **11 Build** | 奢侈品级留白，单一强调色战略使用 | 留白基底，唯一 accent 改 `#2970ff` |
| **03 Information Architects** | 极简内容优先，蓝色超链接传统可对接 | 超链接用 `#2970ff`，保留系统字体阅读优化 |

#### ★☆☆ 需改造（风格色系与金蝶冲突，需大改或谨慎使用）

| 风格 | 冲突点 | 改造代价 |
|------|--------|---------|
| **12 Sagmeister** | 快乐极简=意外色彩爆发（暖色系），与科技蓝冷调冲突 | 需放弃"温暖爆发"，改用蓝色系做克制 accent，风格识别度下降 |
| **15 Ash Thorp** | 温暖赛博（橙/青），橙色 accent 与金蝶主色冲突 | 可改用琥珀 `#ffb200`（金蝶辅助色）替代橙，但需用户认可"金蝶赛博" |
| **09 Experimental Jetset** | 蒙德里安色系（红/蓝/黄），三原色锁定 | 只能取蓝做主色，红/黄需消失或改为紫罗兰 `#7858f9` + 琥珀 `#ffb200` |
| **02 Stamen Design** | 温暖数据可视化色调（赭石/鼠尾草绿/深蓝），整体暖 | 可保留深蓝，但赭石/鼠尾草绿需替换为冰蓝/紫罗兰 |
...
1. **优先从 ★★★ 高适配池选 1-2 个**
2. **从 ★★☆ 中适配池选 1 个做安全备选**
3. **★☆☆ 需改造池仅在用户明确要"突破金蝶风格"时推荐**

**差异化仍需遵守**：3 个方向必须来自不同流派（信息建筑派/运动诗学派/极简主义派/实验先锋派/东方哲学派），不能从同一流派推荐 2 个以上。

**示例推荐组合**：
- 高适配 × 2：04 Fathom（信息建筑派） + 06 Active Theory（运动诗学派）
- 中适配 × 1：01 Pentagram（极简主义派）
→ 覆盖 3 个流派，色系天然兼容，用户选择面够宽
